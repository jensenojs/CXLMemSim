...
#define CXL_GPU_BATCH_DATA_SIZE 0x2000000
...
_Static_assert(sizeof(CXLGPUBatchHtoDHeader) == 32,
               "CXL GPU batch HtoD header size mismatch");
_Static_assert(sizeof(CXLGPUBatchHtoDRange) == 24,
               "CXL GPU batch HtoD range size mismatch");
_Static_assert(offsetof(CXLGPUBatchHtoDHeader, payload_bytes) == 0x10,
               "CXL GPU batch HtoD payload offset mismatch");
...
_Static_assert(CXL_GPU_BATCH_DATA_OFFSET >=
               ...
               "CXL GPU batch payload overlaps the descriptor region");
...
#define CXL_GPU_VERSION 0x00010F00 /* v1.15.0: batch HtoD submission */
...
typedef enum {
    ...
    CXL_GPU_CMD_CACHE_PREFETCH = 0x83, /* Prefetch cache lines into Type-2 cache */
...
#define CXL_BIAS_ENCODE(mode, gran)                                                                                    \
