...
├── bin/
│   ├── cxlmemsim_server
│   └── cuda-integrity-export-oracle
└── guest/
    ├── cxl-gpu-case
    ├── cuda-runtime-dlopen-kernel-probe
    ├── libcuda.so.1
...
- guest构建`libcuda.so.1`、`libcuda.so`、`cxl-gpu-case`和host侧`cuda-integrity-export-oracle`，并运行共享transport、integrity export table与CLI非法参数fixture；同一源码owner使用固定`nvcc`、host C++ compiler与`sm_89`生成tiny probe和library；
...
本任务可以决定CXLMemSim的构建命令、测试目标和七项payload。它不得捆绑系统动态库，不得初始化十五个submodule，不得恢复或构建QEMU、Concordia、kernel、llama或guest，不得用artifact证据关闭Type-2或模型任务。BAR2协议变化由对应跨仓设计和双头一致性验收支配。

下游可依赖的最小事实是：给定正式`repository@digest`，固定puller会恢复上述七项文件图，并证明server、host integrity oracle、shim、case控制CLI、tiny probe和tiny library在声明工具镜像内可装载，library导出`tiny_cuda_launch`，四个局部CTest与guest fixture在build任务通过。下游仍需自行证明这些文件进入实际Type-2运行路径并返回`1234`。
...
- payload恰好七项，类型、mode和link target与profile一致；
- 两次本地归档SHA256一致；
