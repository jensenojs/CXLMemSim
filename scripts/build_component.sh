...
mkdir -p "$BUILD" "$GUEST_SOURCE" "$PAYLOAD/bin" "$PAYLOAD/lib" "$PAYLOAD/guest" "$PAYLOAD/evidence/cuda-api" "$EVIDENCE"
...
bash "$ROOT/scripts/verify_component_payload.sh" "$PAYLOAD" "$EVIDENCE"
