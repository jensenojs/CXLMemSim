...
# agent-usage: bash scripts/package_component.sh --payload-dir EXISTING_PATH --source-commit 40_HEX --work-dir ABSENT_PATH --manifest-out ABSENT_PATH --archive-out ABSENT_PATH
# agent-context: problem=Artifact creation was coupled to registry transport, so a local exact-image build could not sign its bytes without also publishing them.
# agent-context: mental_model=Packaging derives a manifest and deterministic archive from one verified payload and exact clean source commit; registry transport consumes those outputs later.
# agent-context: role=Create and fresh-verify the cxlmemsim artifact manifest and deterministic archive without network access.
# agent-context: use_when=Use after build_component.sh inside the artifact contract exact toolchain image or in the CNB formal build stage.
# agent-context: inputs=Existing payload, exact 40-hex checkout HEAD, absent absolute package work directory, and absent manifest and archive outputs.
# agent-context: outputs=Atomically created manifest.json and component.tar.zst after deterministic-byte and fresh-extraction verification.
# agent-context: interpret=component_package=pass means both terminal outputs identify the verified payload and are ready for publish_component.sh.
# agent-context: proves=The manifest and archive are deterministic for the declared source, profile, contract and payload file graph.
# agent-context: does_not_prove=It does not prove registry transport, fresh pull from OCI, Type-2 execution or Kimi correctness.
# agent-context: next=Pass the exact manifest and archive paths to publish_component.sh, or retain them as a local non-formal artifact output.
...
if [[ ${1:-} == --hint ]]; then
    printf 'agent_hint=self=scripts/package_component.sh\nagent_hint=usage=bash scripts/package_component.sh --payload-dir EXISTING_PATH --source-commit 40_HEX --work-dir ABSENT_PATH --manifest-out ABSENT_PATH --archive-out ABSENT_PATH\nagent_hint=boundary=creates deterministic artifact bytes without registry access\n'
    exit 0
fi
